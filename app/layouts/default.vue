...
const { isOpen: isCartOpen, cartCount, toggleCart, items: cartItems, removeFromCart, cartTotal } = useCart()
...
<template>
  <div class="font-['DM_Sans',sans-serif] bg-[#F9F6F0] text-[#2C241B] min-h-screen flex flex-col relative z-0 selection:bg-[#C5A059] selection:text-white">
    ...
    <header class="fixed top-0 left-0 right-0 z-40 bg-[#F9F6F0]/95 backdrop-blur-md text-[#2C241B] border-b border-[#EAE5DC] shadow-[0_4px_30px_rgba(44,36,27,0.02)]">
      <div class="max-w-[90rem] mx-auto px-4 md:px-8 lg:px-12 h-24 flex items-center justify-between">
        ...
        <div class="flex items-center gap-8">
          ...
          <button @click="toggleCart" class="relative text-[#2C241B] hover:text-[#C5A059] transition-colors duration-300">
            <UIcon name="i-lucide-shopping-bag" class="w-5 h-5 font-light" />
            <span v-if="cartCount > 0" class="absolute -top-1 -right-2 bg-[#C5A059] text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center shadow-sm">
              {{ cartCount }}
            </span>
          </button>
        </div>
    ...
    <div v-if="isCartOpen" class="fixed inset-0 z-[100] flex justify-end">
      <!-- Backdrop -->
      <div class="absolute inset-0 bg-[#2C241B]/20 backdrop-blur-sm transition-opacity" @click="toggleCart"></div>
      ...
      <div class="relative w-full max-w-md h-full bg-[#FDFBF7] shadow-2xl border-l border-[#EAE5DC] flex flex-col transform transition-transform duration-500">
        ...
        <div class="px-8 py-8 flex items-center justify-between border-b border-[#EAE5DC]">
          ...
          <button @click="toggleCart" class="text-[#2C241B]/50 hover:text-[#2C241B] transition-colors">
        ...
        <div class="flex-1 overflow-y-auto px-8 py-6 flex flex-col gap-6">
          <div v-if="cartItems.length === 0" class="flex flex-col items-center justify-center h-full text-[#2C241B]/50 gap-4">
          ...
          <div v-else v-for="item in cartItems" :key="item.product.id" class="flex gap-4 items-center bg-white p-3 rounded-sm border border-[#EAE5DC] shadow-sm">
            <div class="w-20 h-24 bg-[#F9F6F0] rounded-sm overflow-hidden flex-shrink-0 relative">
              <img v-if="item.product.coverImage" :src="item.product.coverImage" class="w-full h-full object-cover" />
            </div>
            <div class="flex-1">
            ...
            <button @click="removeFromCart(item.product.id)" class="text-red-400 hover:text-red-600 transition-colors p-2">
        ...
        <!-- Cart Footer -->
        <div v-if="cartItems.length > 0" class="p-8 border-t border-[#EAE5DC] bg-[#F9F6F0]">
          <div class="flex justify-between items-center mb-6">
            <span class="text-sm uppercase tracking-widest font-semibold text-[#2C241B]/60">Subtotal</span>
            <span class="font-['Fraunces',serif] text-2xl text-[#2C241B]">{{ cartTotal }}</span>
          </div>
          <UButton to="/shop" @click="toggleCart" :ui="{ rounded: 'rounded-sm' }" class="w-full py-4 uppercase tracking-[0.2em] bg-[#2C241B] text-[#F9F6F0] hover:bg-[#C5A059] hover:text-[#2C241B] justify-center text-xs font-bold transition-all duration-300">
            Seguir Comprando
          </UButton>
        </div>
      </div>
